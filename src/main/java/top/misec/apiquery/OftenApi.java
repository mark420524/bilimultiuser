...
public class OftenApi {
     ...
     */
    public static Double getCoinBalance(String cookie) {
        JsonObject responseJson = HttpUtil.doGet(ApiList.getCoinBalance, cookie);
     ...
     */
    public static void getVipPrivilege(int type, ConfigUser user) {
        String requestBody = "type=" + type
                + "&csrf=" + user.getBiliJct();
        JsonObject jsonObject = HttpUtil.doPost(ApiList.vipPrivilegeReceive, requestBody, user.getUserCookie());
     ...
     * @return title
     */
    public static String getVideoTitle(String bvid, ConfigUser user) {
        String title;
        String urlParameter = "?bvid=" + bvid;
        JsonObject jsonObject = HttpUtil.doGet(ApiList.videoView + urlParameter, user.getUserCookie());
     ...
     */
    public static String queryUserNameByUid(String uid, ConfigUser user) {
        String urlParameter = "?mid=" + uid + "&jsonp=jsonp";
        String userName = "1";
        JsonObject jsonObject = HttpUtil.doGet(ApiList.queryUserName + urlParameter, user.getUserCookie());
        if (jsonObject.get("code").getAsInt() == 0) {
