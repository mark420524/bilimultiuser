...
public class GetVideoId {
    ...
    public GetVideoId(String userId, String cookie) {
        this.followUpVideoList = queryDynamicNew(userId, cookie);
        this.rankVideoList = regionRanking(cookie);
        //videoUpdate("14602398",cookie);
    ...
    public void updateAllVideoList(String userId, String cookie) {
        this.followUpVideoList = queryDynamicNew(userId, cookie);
        this.rankVideoList = regionRanking(cookie);
        if (this.followUpVideoList.size() > 0) {
            this.followUpVideoQueue = new ArrayBlockingQueue<>(followUpVideoList.size());
    ...
    public ArrayList<String> queryDynamicNew(String userId,String cookie) {
        ArrayList<String> arrayList = new ArrayList<>();
        String urlParameter = "?uid=" + userId
                + "&type_list=8"
                + "&from="
                + "&platform=web";
        JsonObject jsonObject = HttpUtil.doGet(ApiList.queryDynamicNew + urlParameter,cookie);
     ...
     * 默认请求动画区，3日榜单
     */
    public ArrayList<String> regionRanking( String cookie) {
        int rid = randomRegion();
        int day = 3;
        return regionRanking(rid, day,cookie);
    }
     ...
     * @return 随机返回一个aid
     */
    public ArrayList<String> regionRanking(int rid, int day,String cookie) {

        ArrayList<String> videoList = new ArrayList<>();
        String urlParam = "?rid=" + rid + "&day=" + day;
        JsonObject resultJson = HttpUtil.doGet(ApiList.getRegionRanking + urlParam, cookie);
    ...
    public void videoUpdate(String mid,String cookie){
        String urlParam = "?mid=" + mid + "&ps=30&tid=0&pn=1&keyword=&order=pubdate&jsonp=jsonp";
        JsonObject resultJson = HttpUtil.doGet(ApiList.getBvidByCreate + urlParam,cookie);
        ...
        if (jsonArray != null) {
            for (JsonElement videoInfo : jsonArray) {
                ...
                if(!CoinAdd.isCoinAdded(bvid,cookie)){
