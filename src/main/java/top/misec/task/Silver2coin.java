...
public class Silver2coin implements Task {
    ...
    public void run(ConfigUser user) {
        ...
        JsonObject queryStatus = HttpUtil.doGet(ApiList.getSilver2coinStatus,user.getUserCookie()).get("data").getAsJsonObject();
        //银瓜子兑换硬币汇率
        ...
        if (silverNum < exchangeRate) {
            ...
            String requsetBody = "csrf_token=" + user.getBiliJct() +
                    "&csrf=" + user.getBiliJct();
            JsonObject resultJson = HttpUtil.doPost(ApiList.silver2coin, requsetBody,user.getUserCookie());

            int responseCode = resultJson.get(STATUS_CODE_STR).getAsInt();
            if (responseCode == 0) {
                log.info("银瓜子兑换硬币成功");

                double coinMoneyAfterSilver2Coin = OftenApi.getCoinBalance(user.getUserCookie());
