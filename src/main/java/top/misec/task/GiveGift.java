...
public class GiveGift implements Task {
    ...
    public void run(ConfigUser user) {
        try {
            ...
            /* 获得礼物列表 */
            JsonArray jsonArray = xliveGiftBagList(user);
            /* 判断是否有过期礼物出现 */
            boolean flag = true;
            for (Object object : jsonArray) {
                ...
                if ((expireAt - nowTime) < 60 * 60 * 25 * 1 && expireAt != 0) {
                    /* 如果有未送出的礼物，则获取一个直播间 */
                    if ("".equals(roomId)) {
                        JsonObject uidAndRid = getuidAndRid(user);
                    ...
                    JsonObject jsonObject3 = xliveBagSend(requestBody,user);
     ...
     * @Time 2020-10-13
     */
    public String xliveGetRoomUid(String roomId,ConfigUser user) {
     ...
     * @Time 2020-11-20
     */
    public String getRoomInfoOld(String mid, ConfigUser user) {
        JsonObject pJson = new JsonObject();
     ...
     * @Time 2020-10-13
     */
    public JsonObject xliveBagSend(String requestBody,ConfigUser user) {

        requestBody += "&uid=" + user.getUserId() +
     ...
     * @Time 2020-11-20
     */
    public JsonObject getuidAndRid(ConfigUser user) {
        /* 直播间 id */
        String roomId;
        /* 直播间 uid 即 up 的 id*/
        String uid;
        if (!"0".equals(config.getUpLive())) {
            ...
            roomId = getRoomInfoOld(uid,user);
            String status = "0";
            if (status.equals(roomId)) {
                log.info("自定义up {} 无直播间", uid);
                /* 随机获取一个直播间 */
                roomId = xliveGetRecommend(user);
                uid = xliveGetRoomUid(roomId,user);
                log.info("随机直播间");
            } else {
