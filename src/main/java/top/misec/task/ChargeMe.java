...
public class ChargeMe implements Task {
    ...
    public void run(ConfigUser user) {
        ...
        if (!"0".equals(configChargeUserId)) {
            String userName = OftenApi.queryUserNameByUid(configChargeUserId, user);
        ...
        if (userInfo != null) {
            ...
            JsonObject queryJson = HttpUtil.doGet(ApiList.chargeQuery + "?mid=" + userId,user.getUserCookie());
        ...
        if (day >= 28 && couponBalance >= 2 && Config.getInstance().getMonthEndAutoCharge() ) {
            ...
            JsonObject jsonObject = HttpUtil.doPost(ApiList.autoCharge, requestBody,user.getUserCookie());
    ...
    private void chargeComments(ConfigUser user, String token) {

        String requestBody = "order_id=" + token
                + "&message=" + "BILIBILI-HELPER自动充电"
                + "&csrf=" + user.getBiliJct();
        JsonObject jsonObject = HttpUtil.doPost(ApiList.chargeComment, requestBody, user.getUserCookie());
