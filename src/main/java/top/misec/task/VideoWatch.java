...
public class VideoWatch implements Task {
    ...
    public void watchVideo(String bvid, ConfigUser user) {
        int playedTime = new Random().nextInt(90) + 1;
        String postBody = "bvid=" + bvid
                + "&played_time=" + playedTime;
        JsonObject resultJson = HttpUtil.doPost(ApiList.videoHeartbeat, postBody, user.getUserCookie());
     ...
     */
    public void dailyAvShare(String bvid, ConfigUser user) {
        String requestBody = "bvid=" + bvid + "&csrf=" + user.getBiliJct();
        JsonObject result = HttpUtil.doPost( ApiList.AvShare , requestBody,user.getUserCookie());
