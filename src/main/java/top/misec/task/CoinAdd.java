...
public class CoinAdd implements Task {
    ...
    public void run(ConfigUser user) {
        GetVideoId getVideoId = new GetVideoId(user.getUserId(), user.getUserCookie());
        ...
        //已投的硬币
        int useCoin = TaskInfoHolder.expConfirm(user.getUserCookie());
        //投币策略
        ...
        //投币前硬币余额
        Double beforeAddCoinBalance = OftenApi.getCoinBalance(user.getUserCookie());
        ...
        while (needCoins > 0 && needCoins <= maxNumberOfCoins) {
            ...
            addCoinOperateCount++;
            new VideoWatch().watchVideo(bvid,user);
            boolean flag = coinAdd(bvid, 1, Config.getInstance().getSelectLike(),user);
            if (flag) {
        ...
        log.info("投币任务完成后余额为: " + OftenApi.getCoinBalance(user.getUserCookie()));
    }
     ...
     * @return 是否投币成功
     */
    private boolean coinAdd(String bvid, int multiply, int selectLike,ConfigUser user) {
        String videoTitle = OftenApi.getVideoTitle(bvid, user);
        //判断曾经是否对此av投币过
        if (!isCoinAdded(bvid,user.getUserCookie())) {
            ...
            String requestBody = "bvid=" + bvid
                    ...
                    + "&cross_domain=" + "true"
                    + "&csrf=" + user.getBiliJct();
            JsonObject jsonObject = HttpUtil.doPost(ApiList.CoinAdd, requestBody, headers,user.getUserCookie());
     ...
     * @return 返回是否投过硬币了
     */
    static boolean isCoinAdded(String bvid,String cookie) {
        String urlParam = "?bvid=" + bvid;
        JsonObject result = HttpUtil.doGet(ApiList.isCoin + urlParam,cookie);
